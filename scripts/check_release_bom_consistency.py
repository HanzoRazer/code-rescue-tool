...
def check_release_bom_consistency(bom: dict[str, Any]) -> None:
    ...
    # -----------------------------------------------------------------
    # Treesitter manifest (optional but validated if present)
    # -----------------------------------------------------------------
    if "treesitter_manifest" in artifacts:
        ...
        assert tm_slv == expected_slv, f"{ctx3}.signal_logic_version must match contract_versions"
